...
Progress (2026-01-12): Captured Stripe subscription period end data and applied it to subscription entitlement validThrough timestamps.
...
Progress (2026-01-12): Added role connection update queue + worker to sync Linked Roles metadata on entitlement changes using stored member OAuth tokens (with refresh).
Progress (2026-01-12): Added admin portal Linked Roles setup wizard with step-by-step guidance and metadata registration form.
...
Progress (2026-01-12): Added Storybook test runner to capture component screenshots into a stable storybook-snapshots folder, with scripts to run and update snapshots.
...
Progress (2026-01-12): Added bot runtime env validation helpers for required config values, including URL checks and numeric bounds.
