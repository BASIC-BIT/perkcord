...
const getEncryptionKey = () => {
  ...
  if (key.length !== 32) {
    throw new Error(`${OAUTH_ENCRYPTION_ENV} must be a base64-encoded 32-byte key.`);
  }
  return key;
};
...
export const processRoleConnectionUpdates = action({
  ...
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < limit; index += 1) {
      ...
      try {
        const state = await ctx.runQuery(api.discordRoleConnections.getRoleConnectionState, {
        ...
        const platformUsername =
          state.memberIdentity?.discordUsername?.trim() || update.discordUserId;
        const payload = {
        ...
        await ctx.runMutation(api.discordRoleConnections.completeRoleConnectionUpdate, {
          updateId: update._id,
          status: "completed",
          actorId: "role_connections_worker",
          actorType: "system",
    ...
    return {
      ...
      processedCount: results.filter((item) => item.status === "completed").length,
