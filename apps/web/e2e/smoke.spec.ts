...
test.describe("smoke", () => {
  ...
  test("signed-in admin panels render", async ({ page }, testInfo) => {
    const baseURL =
      ...
      "http://127.0.0.1:3001";
    const sessionSecret =
      process.env.PERKCORD_SESSION_SECRET ?? "playwright-smoke-secret";
    ...
    await expect(
      page.getByRole("heading", { name: "Guild selection" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Force role sync" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Tier management" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Manual grants" })
    ).toBeVisible();
  });
